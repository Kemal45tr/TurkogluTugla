...
using TurkogluTugla.İslemler;
using TurkogluTugla.KullanıcıArayuzu;

namespace TurkogluTugla
{
    public partial class TuglaSatis : Form
    {
        public TuglaSatis()
        {
            InitializeComponent();
        ...
        {
            DataSet ds1 = islem.SatisDetay();
            if (ds1 != null)
                dataGridView1.DataSource = ds1.Tables[0];


            DataSet ds2 = islem.OdemeDetay();
            if (ds2 != null)
        ...
        {
            SatıislarFormu frm = new SatıislarFormu()
            {
                Text = "Satış Yap",
                Satis = new Satis()
                {
        ...
        tekrar:
            ...
            {
                bool b = islem.SatisEkle(frm.Satis);
        ...
        private void btnSatışDüzenle_Click(object sender, EventArgs e)
        {
            ...
            if (sonuc == DialogResult.OK)
            {
                bool b = islem.SatisGüncelle(frm.Satis);
        ...
        private void btnSatışSil_Click(object sender, EventArgs e)
        {
            ...
            if (sonuc == DialogResult.OK)
            {
                bool b = islem.SatisSil(ID);

                if (b)
                {

                    DataSet ds = islem.SatisDetay();
                    if (ds != null)
        ...
        tekrar:
            ...
            if (sonuc == DialogResult.OK)
            {
                bool b = islem.OdemeEkle(frm.Odeme);
        ...
        private void btnÖdemeDüzenle_Click(object sender, EventArgs e)
        {
            ...
            if (sonuc == DialogResult.OK)
            {
                bool b = islem.OdemeGüncelle(frm.Odeme);
        ...
        private void btnÖdemeSil_Click(object sender, EventArgs e)
        {
            ...
            if (sonuc == DialogResult.OK)
            {
                bool b = islem.OdemeSil(ID);

                if (b)
                {

                    DataSet ds = islem.OdemeDetay();
                    if (ds != null)
